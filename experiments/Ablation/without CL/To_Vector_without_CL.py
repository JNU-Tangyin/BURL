...
with custom_object_scope({'AttentionLayer': AttentionLayer}):
    BURL_without_CL = load_model('../../../Save_Model/Full_BURL.h5')
...
encoder = Model(BURL_without_CL.input, BURL_without_CL.layers[-3].output)
...
pd.DataFrame(App_Embedding_by_SentenceBert).to_csv('Spam_Embedding_by_Model_without_CL.csv', index=False)
...
pd.DataFrame(News_Embedding_by_SentenceBert).to_csv('News_Embedding_by_Model_without_CL.csv', index=False)
...
embeddings_App_url = np.array(compressed_representation)[:, 0, :]
App_Embedding_by_SentenceBert = np.hstack((embeddings_App_url, y_resampled.reshape([-1,1])))
pd.DataFrame(App_Embedding_by_SentenceBert).to_csv('Malicious_Phish_Embedding_by_Model_without_CL.csv', index=False)
...
embeddings_App_url = np.array(compressed_representation)[:, 0, :]
App_Embedding_by_SentenceBert = np.hstack((embeddings_App_url, y_resampled.reshape([-1,1])))
pd.DataFrame(App_Embedding_by_SentenceBert).to_csv('Classification_Embedding_by_Model_without_CL.csv', index=False)
...
pd.DataFrame(App_Embedding_by_SentenceBert).to_csv('App_Embedding_by_Model_without_CL.csv', index=False)
